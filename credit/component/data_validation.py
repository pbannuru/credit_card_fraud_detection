    ...
    def validate_dataset_schema(self)->bool:
        try:
            ...
            for col in train_df.columns:
                ...
                if train_df[col].dtype != dataset_schema[SCHEMA_COLUMNS_KEY][col]+'64':
                    validation_status = False
                    break
            
            if validation_status:
                for keys in dataset_schema[DOMAIN_VALUE_KEY].keys():
                    if all(train_df[keys].isin(dataset_schema[DOMAIN_VALUE_KEY][keys])) == True:
                        validation_status = True
                    else:
                        validation_status = False
                        break
