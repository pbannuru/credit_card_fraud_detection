...
import pip
from credit.util.util import read_yaml_file, write_yaml_file
...
from credit.constant import CONFIG_DIR, get_current_time_stamp
...
DEFAULT_PAY_VALUE_KEY = "default_pay"
...
def predict():
    ...
    if request.method == 'POST':
        ...
        default_pay = credit_predictor.predict(X=credit_df)
        if default_pay[0] == 1.0:
            default_text = "Default"
        else:
            default_text = "Not Default"
        context = {
            CREDIT_DATA_KEY: credit_data.get_credit_data_as_dict(),
            DEFAULT_PAY_VALUE_KEY: default_text,
        }
